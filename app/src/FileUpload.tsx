...
const FileUploadPage : React.FC<IProps> = ({share}) => {
    ...
    const changeDesc = (event : any) =>setValue(event.currentTarget.value)
    ...
    const handleSubmission = async() => {
        try {
            ...
            console.log({title,desc:value,hash:added.path})
            console.log(added.path)
            
            share({title,desc:value,hash:added.path})
        } catch (error) {
            ...
            <form method="POST" encType="multipart/form-data">
                ...
                {isFilePicked ? (
                    <div className="remarks">
                        <p>Filename: {selectedFile.name} {selectedFile.type}</p>
                        <p>Size in bytes: {selectedFile.size}</p>
                    </div>
                ) : (
                    <p className="remarks">Select file</p>
                )}
            </form>
